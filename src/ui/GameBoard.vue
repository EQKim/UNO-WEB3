<template>
  <div class="p-6 space-y-3" v-if="room">
    ...
    <div
      ...
      class="mt-4 p-4 rounded-xl border bg-white/70 backdrop-blur"
    >
...
import CardView from "../ui/CardView.vue"; // if your CardView is Vue; adjust path/extension as needed
import {
  subscribeOnlineGame,
  startGameClient,
  playCardOnline,
...
onMounted(() => {
  unsubscribe = subscribeOnlineGame(props.roomId, {
...
const computedIsHost = computed(() =>
  props.isHost ||
  room.value?.hostUid === myUid.value ||
  (players.value.length > 0 && players.value[0]?.id === myUid.value)
);
...
// Standard playable (when no pending stack)
const youHavePlayable = computed<boolean>(
  () => !!top.value && myHand.value.some(c => matches(top.value as Card, c))
);

const winner = computed<string | null>(() => {
  if (!room.value?.winnerUid) return null;
  const w = players.value.find(p => p.id === room.value!.winnerUid);
  return w?.displayName ?? room.value!.winnerUid;
});

const topChosenColor = computed<"red" | "yellow" | "green" | "blue" | null>(() => {
  const t = top.value as any;
  if (!t || t.kind !== "wild") return null;
  return t.chosenColor ?? null;
});
...
async function playChosenWild(color: "red" | "yellow" | "green" | "blue") {
  ...
  try {
    await playCardOnline(props.roomId, toPlay);
  } catch (e: any) {
...
async function onPlayCard(c: Card, i: number) {
  ...
  try {
    await playCardOnline(props.roomId, c);
  } catch (e: any) {
    alert(e?.message ?? String(e));
  }
}
...
async function onDraw() {
  try {
    await drawOneOnline(props.roomId);
  } catch (e: any) {
    alert(e?.message ?? String(e));
  }
}
...
async function onEndTurn() {
  try {
    await endTurnOnline(props.roomId);
  } catch (e: any) {
    alert(e?.message ?? String(e));
  }
}
