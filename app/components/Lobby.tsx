...
import { createRoom, joinRoomByCode, listenRoom } from '@/src/services/Rooms'

type Player = { id: string; displayName: string; isHost?: boolean }

interface LobbyProps {
  onEnterRoom: (payload: { id: string; host: boolean }) => void
}
...
export default function Lobby({ onEnterRoom }: LobbyProps) {
  ...
  useEffect(() => {
    ...
    const stopListen = listenRoom(
      roomId,
      (r: any) => setRoom(r),
      (ps: Player[]) => setPlayers(ps)
    )
  ...
  return (
    <div className="p-3 rounded border border-slate-300 mb-4">
      ...
      <div className="flex gap-2">
        <button onClick={onCreate} className="px-3 py-2 rounded bg-emerald-600 text-white">
          Create room
        </button>
        <input
          value={inviteCode}
        ...
        />
        <button onClick={onJoin} className="px-3 py-2 rounded bg-blue-600 text-white">
          Join
        </button>
      </div>
